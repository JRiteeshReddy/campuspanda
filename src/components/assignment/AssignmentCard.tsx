...
const AssignmentCard = ({ assignment, onUpdate, onDelete }: AssignmentCardProps) => {
  ...
  return (
    <div className="group relative flex items-center justify-between p-4 rounded-lg border border-border dark:border-white/10 bg-background/50 hover:bg-background/80 transition-colors">
      <div className="flex items-center space-x-3">
        ...
        <div>
          ...
          <p className="text-xs font-medium" style={{ color: assignment.completed ? 'rgb(34 197 94)' : statusColor === 'bg-red-500' ? 'rgb(239 68 68)' : statusColor === 'bg-yellow-500' ? 'rgb(234 179 8)' : 'rgb(34 197 94)' }}>
            {statusText}
          </p>
        </div>
      </div>
